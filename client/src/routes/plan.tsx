...
function cellColour(cell: PlannedCell): string {
  ...
  // SelfContained / Overflowing — colour by some heuristic
    ...
    <div className="space-y-6">
            ...
            <form.Field name="region">
              {(field) => (
                <div className="space-y-1">
                  ...
                  <Select
                    value={field.state.value}
                    onValueChange={(v) => field.handleChange(v as FormValues["region"])}
                  >
      ...
      {mutation.isSuccess && mutation.data && (
        <div className="space-y-4">
          ...
          {mutation.data.payload.warnings.length > 0 && (
            <ul className="text-muted-foreground space-y-1 text-sm">
              {mutation.data.payload.warnings.map((w, i) => (
                <li key={i}>⚠ {w}</li>
              ))}
            </ul>
          )}
          <div className="space-y-6">
            {mutation.data.payload.weeks.map((week) => (
